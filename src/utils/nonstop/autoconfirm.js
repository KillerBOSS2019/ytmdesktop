const version = '1.0.0'
const extTag = `[Autoconfirm & Autoplay v${version}]`
...
let confirmDialogElement = 'ytmusic-you-there-renderer'
...
function debug(message) {
    console.debug(`${extTag}[${getTimestamp()}] ${message}`)
}
...
function clickDialog() {
    if (
        document.querySelector(confirmDialogElement).parentElement.style
            .display !== 'none' &&
        !hasActedOnDialog
    ) {
...
function observeDialog() {
    ...
    if (!isObservingDialog) {
        ...
        if (el.length === 1) {
            ...
            log(
                'YouTube changed something in the dialogs...!\nIf you see this message, send a issue at https://github.com/GreepTheSheep/ytmdesktop-adblock-nonstop/issues'
            )
        }
    }
}
...
log(`Monitoring YouTube Music for 'Confirm watching?' action...`)
